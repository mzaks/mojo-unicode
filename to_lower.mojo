...
fn _to_lower(a: UInt8, b: UInt8) -> (Int, Int, Int, Int):
    ...
    if a == 195:
        ...
        if b == 184:
            return ((254), (7), 0, 2)
        alias special = SIMD[DType.uint8, 8](
            UInt8(129),
            UInt8(131),
            UInt8(133),
            UInt8(135),
        ...
        if b == 129:
            return ((3), (18), 0, 2)
        alias special_1 = SIMD[DType.uint8, 16](
            UInt8(130),
            UInt8(132),
            UInt8(135),
            UInt8(139),
        ...
        return (
            (0),
            (1) * int(is_special | (l1 & h1 & odd) | (l2 & h2 & ~odd)),
            0,
            2,
        )
    elif a == 200:
        ...
        var v = (special_1 == b).reduce_or()
        return ((0), (1) * int(v), 0, 2)
    elif a == 205:
...
fn lower_utf8(p: UnsafePointer[UInt8], bytes: Int) -> String:
    ...
    var result = UnsafePointer[UInt8].alloc(capacity)
    var added_bytes = 0
    var cp = p
    ...
    while rest_bytes > 0:
        var char_length = int(
            (cp.load() >> 7 == 0).cast[DType.uint8]() * 1
    ...
    resultp[0] = 0
    var list = List[UInt8](
        unsafe_pointer=result, size=(added_bytes + 1), capacity=capacity
    )
    return String(list)
